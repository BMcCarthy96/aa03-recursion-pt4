...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
    ...
    const findBestChange = (remainingTarget, currentChange, coinIndex) => {
        if (remainingTarget === 0) {
            if (
                bestChange === null ||
                currentChange.length < bestChange.length
            ) {
                bestChange = currentChange.slice();
            }
            return;
        }
        ...
        for (let i = coinIndex; i < coins.length; i++) {
            ...
            if (coin <= remainingTarget) {
                ...
                findBestChange(remainingTarget - coin, currentChange, i);
                currentChange.pop();
            }
        }
    };

    findBestChange(target, [], 0);
